...
But how do we sort the elements by their frequencies in O(n) time? My first thought instead of calling on sorted, is that we do during the single pass, we sort the elements by their frequencies.

How can we do this?
- Bucket Sort: We can use a bucket sort to sort the elements by their frequencies in O(n) time, as we iterate through the array.

Bucket Sort Example:
- We have the array [1, 1, 1, 2, 2, 3]
- We can use a bucket sort to sort the elements by their frequencies in O(n) time, as we iterate through the array.
- We can use a dictionary to count the frequencies of the elements in the array.
- We can then use a bucket sort to sort the elements by their frequencies in O(n) time.
- We can then return the top k elements from the bucket sort.
- This is a O(n) time and O(n) space solution.

Ex. [1, 1, 1, 2, 2, 3]
Bucket Sort: [[3, 1], [2, 2], [1, 3]], but how do we get the keys from the bucket sort? How do we get the top k elements from the bucket sort?
