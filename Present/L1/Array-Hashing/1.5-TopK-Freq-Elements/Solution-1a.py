...
class Solution: # Built-In
    def helper(self, x):
        return x[1]  # Return the frequency (value) for sorting
    ...
    def topKFrequent(self, nums: List[int], k: int) -> List[int]:
        ...
        # sorted() returns list of (key, value) tuples like [(3, 3), (2, 2), (1, 1)]
        sortedDict = sorted(numDict.items(), key=self.helper, reverse=True)
        
        # Extract only the keys (the numbers) from the first k items
        for i in range(k):
            returnArray[i] = sortedDict[i][0]  # [0] gets the key (number), [1] would be frequency
